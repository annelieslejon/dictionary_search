...
def cosine_similarity(emb_1, emb_2):
    ...
    return np.dot(emb_2, emb_1) / (np.linalg.norm(emb_1) * np.linalg.norm(emb_2))

class SearchEngine:
    """Allows searching the database."""

    def __init__(self, database_path: str, max_entries: int,query_set):
        self.database = []
    ...
    def get_results(self, embedding: np.ndarray,random_embedding) -> List[Tuple[str, float]]:
        ...
        for i, key in enumerate(self.database):
            cossim = cosine_similarity(embedding, key.embedding)
            dist=  1-cossim # np.linalg.norm(embedding - key.embedding) #
            score = (1+cossim)/2
            distances.append((dist, i,score))
        ordered = sorted(distances, key=lambda tup: tup[0])

        results = [(self.database[i].gloss, i, float(s),float(d)) for (d, i,s) in ordered]
