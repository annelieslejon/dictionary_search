...
import numpy as np
from dictionary_search_model import Model, load_video, load_video_np, preprocess_video
...
def _process(video_file: str, output_directory: str, model_path: str, extension: str) -> bool:
    ...
    try:
        if(video_file.endswith('.npy')):
            features: np.array = load_video_np(video_file)
        else:
...
def main(input_directory: str, output_directory: str, model_path: str, extension: str, jobs: int):
    ...
    print('nof video_files' + str(len(video_files)))
